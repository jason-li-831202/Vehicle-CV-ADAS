...
class ObjectTrackBase(metaclass=ABCMeta):
    ...
    def plot_directions(self, img: np.ndarray, init_point: list, observations: list, class_id: int) -> None:
        ...
        if (len(observations ) > 1 ):
            ...
            # TODO: Judgment start indicator box
            # if (not observations.full()):
            #     rate_w = (cx - (cx-w//2))/observations._maxlen
            #     rate_h = (cy - (cy-h//2))/observations._maxlen

            #     sx = int(cx-w//2 + rate_w*len(observations) )
            #     sy = int(cy-h//2 + rate_h*len(observations) )
            #     ex = int(cx+w//2 - rate_w*len(observations) )
            #     ey = int(cy+h//2 - rate_h*len(observations) )
            #     cv2.rectangle(img, (sx, sy), (ex, ey), tuple( i-10 for i in self.class_colors[class_id]), 1, cv2.LINE_AA) 
